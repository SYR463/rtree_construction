...
public class RTreeFromFile {
    public static void main(String[] args) {
        // 输入文件夹路径（批量读取CSV文件）
        String inputFolder = "us101Data/01_processedData/";
        String outputFolder = "us101Data/02_RTreeString/";  // 输出R*-tree的结构
...
//        String outputFolder = "preExperData/testDataset/RTreeString/";  // 输出R*-tree的结构
        ...
        for (File csvFile : files) {
            ...
            // 创建 R*-tree
            RTree<String, Rectangle> tree = RTree.minChildren(4).maxChildren(8).splitter(splitterRStar).create();
            ...
            try (BufferedReader br = new BufferedReader(new FileReader(csvFile))) {
                ...
                while ((line = br.readLine()) != null) {
                    ...
                    if (values.length < 8) continue; // 确保数据完整，【原始数据中的每一行有8列】
